  ...
  modules: {
      ...
      init: async () => {
        const [
          ...
          supabaseClient
            .from("asset_movements")
            .select(
              "equipment_id, equipment(name), asset_number, serial_number, received_by, moved_at, destination_room_id, moved_by",
            )
            .order("moved_at", { ascending: false }),
          supabaseClient.from("rooms").select("id, name"),
          supabaseClient.from("profiles").select("id, full_name"),
    ...
    movimentacoes: {
      ...
      update: async (e, movId) => {
        ...
        const { error } = await supabaseClient
          ...
          .update({
            ...
            asset_number:
              document.getElementById("edit-mov-asset").value.trim() || null,
      ...
      create: async (e) => {
        ...
        const assetNumber =
          document.getElementById("mov-asset-number").value || null;
        const receivedBy =
...
App.scanner = {
  ...
  open: async () => {
    ...
    try {
      ...
      App.scanner._startBarcode(video);
      App.scanner._startOCR(video);
    } catch (err) {
      UI.showToast("Erro ao acessar câmera.", "danger");
      App.scanner.close();
    }
  },

  /* ── BARCODE LOOP ───────────────────── */
  _startBarcode: async (video) => {
    ...
    const loop = async () => {
      ...
      App.scanner._animFrame = requestAnimationFrame(loop);
    };
  ...
  _onSuccess: async (decoded) => {
    ...
    UI.showToast(`Patrimônio detectado: ${assetNumber}`, "success");
