  ...
  handleRoute: () => {
    ...
    switch (route) {
      ...
      case "#movimentacoes":
        ...
        break;
      case "#rastreio":
        App.modules.rastreio.init();
        break;
      case "#mapa-salas":
        App.modules.mapaSalas.init();
        break;
      case "#perfil":
        App.modules.perfil.init();
        break;
      default:
  ...
  modules: {
    ...
    workflow: {
      ...
      showCreateModal: async () => {
        const { data: rooms } = await supabaseClient
          .from("rooms")
          .select("id, name");
        document.getElementById("modal-root").innerHTML = Views.app.ticketModal(
          rooms || [],
        );
        if (typeof lucide !== "undefined") lucide.createIcons();
      },
      ...
      deleteTicket: async (ticketId) => {
        ...
        if (btn.dataset.confirming) {
          ...
          btn.innerHTML =
            '<i data-lucide="alert-triangle"></i> Confirmar exclusão';
          if (typeof lucide !== "undefined") lucide.createIcons();
          setTimeout(() => {
            if (btn.dataset.confirming) {
              delete btn.dataset.confirming;
      ...
      updateRoom: async (e, salaId) => {
        ...
        const { error } = await supabaseClient
          ...
          .update({
            ...
            room_number:
              document.getElementById("edit-room-number").value || null,
            coordinator:
              document.getElementById("edit-room-coordinator").value || null,
          })
          .eq("id", salaId);
        if (error) {
    ...
    usuarios: {
      ...
      createUsuario: async (e) => {
        ...
        const full_name = document
          .getElementById("create-usuario-name")
          .value.trim();
        const email = document
          .getElementById("create-usuario-email")
          .value.trim();
        ...
        UI.showToast(
          `Usuário "${full_name}" criado com senha padrão Fundepar26.`,
          "success",
        );
        App.modules.usuarios.init();
      },
      ...
      updateUsuario: async (e, userId) => {
        ...
        const { error } = await supabaseClient
          ...
          .update({
            full_name: document
              .getElementById("edit-usuario-name")
              .value.trim(),
      ...
      resetSenha: async (btn, userId, userName, userEmail) => {
        if (btn.dataset.confirming) {
          ...
          if (ok) {
            UI.showToast(
              `Email de redefinição de senha enviado para "${userName}".`,
              "success",
            );
          }
          btn.disabled = false;
    ...
    equipamentos: {
      ...
      applyFilters: () => {
        ...
        const status =
          document.getElementById("equip-filter-status")?.value || "";
      ...
      updateEquipamento: async (e, id) => {
        ...
        const { error } = await supabaseClient
          ...
          .update({
            ...
            categoria:
              document.getElementById("edit-equip-categoria").value.trim() ||
              null,
            status: document.getElementById("edit-equip-status").value || "bom",
            observacao:
              document.getElementById("edit-equip-observacao").value.trim() ||
              null,
          })
          .eq("id", id);
        if (error) {
    ...
    rastreio: {
      ...
      init: async () => {
        const [
          ...
          supabaseClient
            .from("equipment")
            .select("id, name, categoria, status, observacao")
            .order("name"),
          supabaseClient
            .from("asset_movements")
            .select(
      ...
      applyFilters: () => {
        const term = (document.getElementById("rastreio-search")?.value || "")
          ...
          .trim();
        const roomId =
          document.getElementById("rastreio-filter-room")?.value || "";
        const categoria =
        ...
        if (sort === "az")
          filtered.sort((a, b) =>
            (a.equipment?.name || "").localeCompare(
              b.equipment?.name || "",
              "pt-BR",
            ),
          );
          ...
          filtered.sort((a, b) =>
            (b.equipment?.name || "").localeCompare(
              a.equipment?.name || "",
              "pt-BR",
            ),
          );
          ...
          filtered.sort((a, b) =>
            (a.asset_number || "zzz").localeCompare(
              b.asset_number || "zzz",
              "pt-BR",
              { numeric: true },
            ),
      ...
      exportExcel: () => {
        ...
        XLSX.writeFile(
          wb,
          `rastreio_${new Date().toISOString().slice(0, 10)}.xlsx`,
        );
    ...
    movimentacoes: {
      ...
      renderPage: () => {
        ...
        if (tbody) {
          tbody.innerHTML = Views.app.movimentacoesRows(slice);
          if (typeof lucide !== "undefined") lucide.createIcons();
        }
        ...
        if (countEl)
          countEl.textContent = `${mod._filteredData.length} resultado${mod._filteredData.length !== 1 ? "s" : ""}`;
      },

      prevPage: () => {
        if (App.modules.movimentacoes._page > 1) {
          App.modules.movimentacoes._page--;
      ...
      nextPage: () => {
        const mod = App.modules.movimentacoes;
        if (mod._page < Math.ceil(mod._filteredData.length / mod._pageSize)) {
          mod._page++;
          mod.renderPage();
          window.scrollTo(0, 0);
        }
      ...
      handleImportFile: async (input) => {
        ...
        const rows = raw.map((row, i) => {
          ...
          const serialNumber =
            (row["Nº Série"] || "").toString().trim() || null;
          const assetNumber =
            (row["Nº Patrimônio"] || "").toString().trim() || null;
          ...
          if (!destName) errors.push("Destino não informado");
            ...
            errors.push("Sala de destino não encontrada no cadastro");
          if (originName && !origin)
            warnings.push("Sala de origem não encontrada — ficará em branco");
          if (!movedAt)
            warnings.push(
              "Data inválida ou ausente — será usada a data/hora atual",
            );
      ...
      applyFilters: () => {
        ...
        const assetDigitsFilter = assetFilter.replace(/\D/g, "");
        ...
        const filtered = App.modules.movimentacoes._lastData.filter((m) => {
          ...
          // 🔎 FILTRO DE PATRIMÔNIO
          if (assetDigitsFilter) {
            const movementDigits = (m.asset_number || "").replace(/\D/g, "");
            if (!movementDigits.includes(assetDigitsFilter)) return false;
          }
      ...
      init: async () => {
        ...
        movimentacoes.forEach((m) => {
          if (m.equipment_id && m.equipment?.name)
            eqMap.set(m.equipment_id, {
              id: m.equipment_id,
              name: m.equipment.name,
            });
        });
        const uniqueEquipment = [...eqMap.values()].sort((a, b) =>
          a.name.localeCompare(b.name, "pt-BR"),
        );

        document.getElementById("view-content").innerHTML =
          Views.app.movimentacoes(
            movimentacoes,
            uniqueEquipment,
            rooms || [],
            profilesList || [],
      ...
      lookupAndFillOrigin: async (rawValue) => {
        ...
        const { data } = await supabaseClient
          .from("asset_movements")
          .select(
            "destination_room_id, destination_room:destination_room_id(id, name)",
          )
        ...
        if (data && data.length > 0 && data[0].destination_room) {
          ...
          UI.showToast(
            `Origem preenchida automaticamente: ${room.name}`,
            "success",
          );
        }
      },
      ...
      showEditModal: async (movId) => {
        const movement = App.modules.movimentacoes._lastData.find(
          (m) => m.id === movId,
        );
        if (!movement) return;
        const { data: rooms } = await supabaseClient
      ...
      update: async (e, movId) => {
        ...
        const originRoomId =
          document.getElementById("edit-mov-origin-id").value;
        const destRoomId = document.getElementById(
          "edit-mov-destination-id",
        ).value;
        const editReason = document
          .getElementById("edit-mov-reason")
          .value.trim();

        const originRoom = App.modules.movimentacoes._rooms.find(
          (r) => r.id === originRoomId,
        );
        const destRoom = App.modules.movimentacoes._rooms.find(
          (r) => r.id === destRoomId,
        );
        ...
        if (originRoomId === destRoomId) {
          UI.showToast(
            "A sala de origem e destino não podem ser iguais.",
            "warning",
          );
          btn.disabled = false;
          btn.textContent = orig;
        ...
        const rawEditAsset = document
          .getElementById("edit-mov-asset")
          .value.trim();
        ...
        const { error } = await supabaseClient
          ...
          .update({
            serial_number:
              document.getElementById("edit-mov-serial").value.trim() || null,
            asset_number: editAsset,
            origin_room_id: originRoom.id,
            destination_room_id: destRoom.id,
            received_by:
              document.getElementById("edit-mov-received-by").value.trim() ||
              null,
            moved_at: movedAtVal
              ? new Date(movedAtVal).toISOString()
              : undefined,
            is_edited: true,
      ...
      showEditInfo: async (movId) => {
        const [{ data: logs, error }, { data: profilesList }] =
          await Promise.all([
            supabaseClient
              .from("movement_edits")
              .select("*")
      ...
      fillEquipmentData: (name) => {
        const eq = App.modules.movimentacoes._equipment.find(
          (e) => e.name === name,
        );
      ...
      create: async (e) => {
        ...
        const serialNumber =
          document.getElementById("mov-serial").value || null;
        const rawAsset =
          document.getElementById("mov-asset-number").value || "";
        ...
        const assetNumber =
          assetDigits.length === 12
            ...
            : rawAsset || null;
        const receivedBy =
          document.getElementById("mov-received-by").value.trim() || null;
        ...
        if (assetNumber) {
          ...
          if (lastMov && lastMov.length > 0) {
            if (lastMov[0].destination_room_id === destRoomId) {
              UI.showToast(
                "Este patrimônio já está na sala de destino selecionada.",
                "warning",
              );
              btn.disabled = false;
              btn.textContent = orig;
        ...
        const itemStatus =
          document.getElementById("mov-item-status").value || null;
        const comentario =
          document.getElementById("mov-comentario").value.trim() || null;
